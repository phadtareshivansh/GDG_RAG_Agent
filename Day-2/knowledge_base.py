...
class KnowledgeBase:
    ...
    def __init__(self, collection_name: str = "gdg_knowledge"):
        ...
        # Attempt to initialize the SentenceTransformer embedding function.
        # If the environment lacks PyTorch or the model, fall back to a
        # deterministic lightweight embedding function so the KB remains usable
        ...
        try:
            ...
            # Fallback deterministic embedding function using seeded RNG
            print("⚠️ Could not load SentenceTransformer embeddings, using fallback deterministic embeddings:", e)
            ...
            class FallbackEmbeddingFunction:
                ...
                def name(self) -> str:
                    # Provide a name method expected by chromadb's validation
                    return "fallback-deterministic"

                def _vec_from_text(self, text: str):
                    # Use a hash of the text to seed random generator so outputs are deterministic
                ...
                def __call__(self, input):
                    """
                    ChromaDB expects embedding functions to be callable with a single
                    `input` argument (either a single string or a list of strings).
                    Return a list of vectors for a list input, or a single vector
                    for a single string.
                    """
                    # If a single string was passed, return a single vector
                    if isinstance(input, str):
                        return self._vec_from_text(input)
                    # Otherwise, assume iterable of strings
                    return [self._vec_from_text(t) for t in input]

                # Backwards-compatible helpers
                def embed_documents(self, input):
                    # Chroma may call embed_documents(input=...) where input is a list
                    texts = input if isinstance(input, list) else list(input)
                    return [self._vec_from_text(t) for t in texts]

                def embed_query(self, input):
                    # Chroma may call embed_query(input=...) where input is a single string
                    if isinstance(input, str):
                        return self._vec_from_text(input)
                    # If a list is passed, return embeddings for each
                    return [self._vec_from_text(t) for t in input]

            self.embedding_function = FallbackEmbeddingFunction(dim=64)
            self._embedding_dim = 64
            self._embedding_model = 'fallback-deterministic-64'
        
        # TODO: Create or get collection
        # Create/get collection; pass the embedding function (fallback or real)
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'embedding_dimension': getattr(self, '_embedding_dim', 0),
            'embedding_model': getattr(self, '_embedding_model', 'unknown')
        }
