...
# Ensure the actual Day-2 folder is on sys.path so imports like `from knowledge_base import KnowledgeBase` work
...
class RAGAgent:
    ...
    def __init__(
        ...
        # Initialize Gemini with RAG-optimized settings (be resilient if key/SDK missing)
        try:
            self.llm = GeminiWrapper(api_key=gemini_api_key, model_name="gemini-2.5-flash", temperature=temperature)
        except Exception as e:
            print(f"⚠️ Gemini client not available or API key missing: {e}")
            self.llm = None
        
        # Set a persona for accurate, sourced responses
        # Only set the persona if the Gemini client initialized successfully
        persona_text = (
        ...
        if self.llm is not None:
            try:
                self.llm.set_persona(persona_text)
            except Exception:
                # Persona is best-effort; continue even if it fails
                print("⚠️ Unable to set persona on Gemini client; continuing without persona.")
        else:
            print("⚠️ Gemini client not configured. Persona not set; generation will be disabled.")
    ...
    def set_knowledge_base(self, knowledge_base: KnowledgeBase):
        ...
        """
        # attach the provided knowledge base
        self.knowledge_base = knowledge_base
        stats = knowledge_base.get_stats() if knowledge_base is not None else {'collection_name': 'none', 'total_chunks': 0}
...
Your answer:"""
    ...
    def answer(self, query: str, top_k: int = 3, verbose: bool = True) -> Dict:
        ...
        1) Retrieve context, 2) Build prompt, 3) Generate answer via LLM.
        Returns a dict with the answer and sources.
        """
        ...
        if self.llm is None:
            answer_text = "[LLM not configured] Cannot generate answer because Gemini client is not available."
        else:
            # GeminiWrapper.generate returns a string response
            try:
